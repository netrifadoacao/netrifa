...
async function main() {
  ...
  const sql = fs.readFileSync(path.join(__dirname, '..', 'supabase', 'seed-extra-members.sql'), 'utf8');
  await client.query(sql);
  await client.end();
  console.log('Seed extra (membros 4-20) aplicado no banco remoto. Senha: Senha123!');
}
