...
async function main() {
  ...
  const sql = fs.readFileSync(path.join(__dirname, '..', 'supabase', 'fix-auth-users.sql'), 'utf8');
  const res = await client.query(sql);
  await client.end();
  console.log('auth.users atualizado: tokens NULL viraram string vazia. Tente logar de novo.');
}
